...
基于 **Next.js 15 (App Router) + TypeScript + Tailwind CSS + SQLite (sql.js)** 的轻量化虚拟购物平台。所有交易使用虚拟币，无真实资金。内置 AI 购物助手，支持 Function Calling 智能下单。
...
| 数据库 | SQLite WASM (`sql.js`) — 零依赖，本地文件 `data/shopfree.db` |
...
运行 `npm run seed` 会导入全部 40 件商品并创建"官方商城"卖家。
